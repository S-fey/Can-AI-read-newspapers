#!/usr/bin/env python3
"""
Ask GPT-5.5 to answer newspaper front-page questions from a PDF.
...
Usage:
    python analyze_newspaper.py newspaper.pdf
    python analyze_newspaper.py newspaper.pdf --prompt prompt.txt --template template.json --output answer.json
"""
...
def pdf_to_data_url(pdf_path: Path) -> str:
    """Read a PDF file and return it as a base64 data URL."""
...
def ask_task(client: OpenAI, base_instructions: str, filename: str, pdf_data_url: str, task_items: list) -> dict:
...
def main():
    ...
    for i, task_items in enumerate(tasks, start=1):
        ...
        try:
            answers = ask_task(client, base_instructions, pdf_path.name, pdf_data_url, task_items)
